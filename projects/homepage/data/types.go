...
type Project struct {
	...
	Diagram    string           `json:"diagram,omitempty"` // Mermaid graph definition
...
// LongDescFor returns the localized detail-page long description, falling
// back to the short DescFor text for projects without dedicated content.
func (p Project) LongDescFor(lang string) string {
	if lang == "de" && p.LongDescDE != "" {
		return p.LongDescDE
	}
	if p.LongDesc != "" {
		return p.LongDesc
	}
	return p.DescFor(lang)
}

// HasDetailPage reports whether this project should link to a /projects/<id>
// detail page: it needs authored long-form content, and live-tool projects
// (regex/cron/codec) are excluded since they're already standalone apps at
// their own subdomain rather than portfolio cards.
func (p Project) HasDetailPage() bool {
	if p.LongDesc == "" {
		return false
	}
...
func TechIcon(keyword string) string {
	m := map[string]string{
		...
		"data analysis":     "lucide:bar-chart-2",
		"data-analysis":     "lucide:bar-chart-2",
		"algorithms":        "lucide:puzzle",
		"puzzles":           "lucide:puzzle",
		"simulation":        "lucide:cpu",
		"sync":              "lucide:refresh-cw",
		"ui":                "lucide:layout-grid",
		"tex":               "lucide:scroll",
		"typst":             "lucide:scroll",
